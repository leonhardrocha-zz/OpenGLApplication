...
void OpenGL21Renderer::LoadGLTextures(std::string directory) 
{
	ILboolean success;
	...
	{
		//save IL image ID
		std::string filename = directory + (*itr).first;  // get filename
...
void OpenGL21Renderer::ApplyMaterial(const aiMaterial *mtl)
{
	float c[4];
...
void OpenGL21Renderer::RecursiveRender (const aiScene *sc, const aiNode* nd)
{
	unsigned int i;
	unsigned int n = 0, t;
	...
	glPushMatrix();
	memcpy(modelviewMatrix,&m,sizeof(float) * 16);
