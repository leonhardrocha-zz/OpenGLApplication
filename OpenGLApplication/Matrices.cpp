...
Matrix4& Matrix4::LookAt(const Vector3& vecPos, const Vector3& vecLookAt, bool Reset)
{
	Vector3 X, Y, Z;	
	Z = vecPos - vecLookAt; 
	Z.normalize();
	X = Vector3(0.0f, 1.0f, 0.0f).cross(Z);
	float magnitude = X.length();
	if ( magnitude < 0.01f)
	{
		X = Vector3(0.0f, 0.0f, 1.0f).cross(Z);
		magnitude = X.length();
	}
	if ( magnitude < 0.01f)
	{
	...
	Matrix4 view(X.x, Y.x, Z.x, 0.0f,
				 X.y, Y.y, Z.y, 0.0f,
				 X.z, Y.z, Z.z, 0.0f,
				 -vecPos.dot(X), -vecPos.dot(Y), -vecPos.dot(Z), 1.0f);	 
